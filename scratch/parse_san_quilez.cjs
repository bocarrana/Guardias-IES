...
console.log(`Parsed ${parsed.length} teachers.`);

// Generate SQL
let sql = `-- Carga de Profesores: IES Sierra de San Quílez (Binéfar)\n`;
sql += `DELETE FROM "Profesores";\n\n`;
sql += `INSERT INTO "Profesores" (id, "nombre y apellidos", email, departamento, "grupo de guardia", rol, horas_guardia, activo)\nVALUES\n`;

const values = parsed.map(p => {
  const n = p.nombre.replace(/'/g, "''");
  const e = p.email.replace(/'/g, "''");
  const d = p.departamento.replace(/'/g, "''");
  const g = p.guardGroup.replace(/'/g, "''");
  const r = p.rol.replace(/'/g, "''");
  return `  ('${p.id}', '${n}', '${e}', '${d}', '${g}', '${r}', 1, true)`;
});

sql += values.join(',\n') + ';\n';

fs.writeFileSync('ies_sierra_de_san_quilez/carga_profesores_san_quilez.sql', sql);
console.log('Saved: ies_sierra_de_san_quilez/carga_profesores_san_quilez.sql');

// Generate CSV
let csv = `"nombre y apellidos","email","departamento","grupo de guardia","rol","horas_guardia","activo"\n`;
csv += parsed.map(p => `"${p.nombre}","${p.email}","${p.departamento}","${p.guardGroup}","${p.rol}",1,true`).join('\n');

fs.writeFileSync('ies_sierra_de_san_quilez/profesores_san_quilez.csv', csv);
console.log('Saved: ies_sierra_de_san_quilez/profesores_san_quilez.csv');
