...
import { Teacher, Guard, GuardStatus, GuardType, TimeSlot, Classroom, Group, Subject } from '../types';
...
export const getTeacherByEmail = async (email: string): Promise<Teacher | null> => {
  if (supabase) {
    ...
    if (error) {
        console.error("Error buscando profesor por email:", error.message);
        return null;
    }
    if (!data) return null;
    ...
    return {
        id: data.id,
        name: data['nombre y apellidos'],
        email: data.email,
        department: data.department,
        guard_group: data['grupo de guardia'],
...
export const getGuards = async (): Promise<Guard[]> => {
  if (supabase) {
    console.log("Cargando guardias con esquema actualizado...");
    
    // Consulta con nombres exactos y relaciones
    const { data, error } = await supabase
      .from('Guardias')
      .select(`
        "ID Guardia",
    ...
    if (error) {
        console.error("SUPABASE ERROR CARGANDO GUARDIAS:", error);
        return [];
    }
    ...
    return data.map((g: any) => ({
        ...
        requesting_teacher: g.requesting_teacher ? {
            id: g.requesting_teacher.id,
            name: g.requesting_teacher['nombre y apellidos'],
            avatar_url: g.requesting_teacher.foto,
            department: '', guard_group: ''
        } : undefined,
        
        covering_teacher: g.covering_teacher ? {
            id: g.covering_teacher.id,
            name: g.covering_teacher['nombre y apellidos'],
            avatar_url: g.covering_teacher.foto,
            department: '', guard_group: ''
        } : undefined,
        ...
        group: g.group ? {
            id: g.group['id grupos'],
            name: g.group['grupos']
        } : undefined,
        
        subject: g.subject ? {
            id: g.subject['id materias'],
            name: g.subject['materias']
        } : undefined,

        classroom: g.classroom ? {
            id: g.classroom['id aulas'],
            name: g.classroom['aulas']
        } : { id: g['Aula'], name: g['Aula'] } // Fallback si no hay relacion
...
export const createGuard = async (guard: Partial<Guard>): Promise<any> => {
  if (supabase) {
    ...
    // Mapeo EXACTO a columnas de la base de datos
    const dbPayload = {
        "ID Guardia": newId,
        "Fecha": guard.date,
        ...
        "Profesor ausente": guard.requesting_teacher_id,
        "Estado": GuardStatus.AVAILABLE,
        "Tipo de Guardia": guard.type,
        "Tarea dejada": guard.has_task || 'No',
        "Observaciones": guard.observations
    };
...
export const getMetaOptions = async () => {
   if(supabase) {
       ...
       const slots = results[0].status === 'fulfilled' && results[0].value.data ? results[0].value.data : [];
       const classrooms = results[1].status === 'fulfilled' && results[1].value.data ? results[1].value.data : [];
       const groups = results[2].status === 'fulfilled' && results[2].value.data ? results[2].value.data : [];
       const subjects = results[3].status === 'fulfilled' && results[3].value.data ? results[3].value.data : [];

       // Mapeo usando los nombres de columna del nuevo esquema
